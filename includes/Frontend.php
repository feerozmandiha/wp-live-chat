...
class Frontend {
    ...
    public function handle_send_chat_message(): void {
        error_log('WP Live Chat: handle_send_chat_message called');
        
        $this->verify_nonce('send_chat_message');
        
        $session_id = sanitize_text_field($_POST['session_id'] ?? '');
        $message = sanitize_textarea_field($_POST['message'] ?? '');
        $step = sanitize_text_field($_POST['step'] ?? 'general_message');
        $temp_id = sanitize_text_field($_POST['temp_id'] ?? '');
        
        error_log("WP Live Chat: send_chat_message - session_id: {$session_id}, step: {$step}, message_length: " . strlen($message));
        ...
        try {
            ...
            error_log("WP Live Chat: Message saved with ID: {$message_id}");
            
            // اگر conversation flow active است، پردازش کن
            $flow_result = null;
            if ($step !== 'general_message') {
                try {
                    $flow = new Conversation_Flow($session_id);
                    $flow_result = $flow->process_input($message, $step);
                    
                    error_log("WP Live Chat: Flow processed: " . json_encode($flow_result, JSON_UNESCAPED_UNICODE));
                    
                    if ($flow_result['success']) {
                        // ارسال پیام سیستم به کاربر
                        $pusher_service = Plugin::get_instance()->get_service('pusher_service');
                        if ($pusher_service) {
                            $pusher_service->trigger("chat-{$session_id}", 'new-message', [
                                'id' => 'sys_' . time(),
                            ...
                            error_log("WP Live Chat: System message sent via Pusher");
                        }
                    }
                } catch (\Exception $e) {
                    error_log("WP Live Chat: Error in flow processing: " . $e->getMessage());
                    // به پردازش ادامه بده حتی اگر flow با خطا مواجه شد
                }
            }
            ...
            error_log("WP Live Chat: Error in send_chat_message: " . $e->getMessage());
            wp_send_json_error('خطای سرور: ' . $e->getMessage());
        }
    }
